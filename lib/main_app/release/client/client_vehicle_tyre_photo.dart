...
class ClientVehicleTyrePhotoPage extends StatefulWidget {
  const ClientVehicleTyrePhotoPage({Key? key}) : super(key: key);

  @override
  State<ClientVehicleTyrePhotoPage> createState() => _VehicleQRScanPageState();
}

class _VehicleQRScanPageState extends State<ClientVehicleTyrePhotoPage> {
  String photoLocation = "";
  File? photoLeftFrontTyreFile;
  File? photoRightFrontFile;
  File? photoLeftBackTyreFile;
  File? photoRightBackTyreFile;
  ...
  uploadCameraFrontLeftTyre() async {
    ...
    if (image != null) {
      ...
      setState(() => photoLeftFrontTyreFile = imageTemporary);
  ...
  uploadCameraRightFrontTyre() async {
    ...
    if (image != null) {
      ...
      final imageTemporary = File(image.path);
      setState(() => photoRightFrontFile = imageTemporary);
      // Navigator.of(context);
    } else {
      print('No Image Path Received');
    }
  ...
  uploadCameraLeftBackTyre() async {
    ...
    if (image != null) {
      ...
      final imageTemporary = File(image.path);
      setState(() => photoLeftBackTyreFile = imageTemporary);
      // Navigator.of(context);
    } else {
      print('No Image Path Received');
    }
  ...
  uploadCameraRightBackTyre() async {
    ...
    if (image != null) {
      ...
      final imageTemporary = File(image.path);
      setState(() => photoRightBackTyreFile = imageTemporary);
      // Navigator.of(context);
    } else {
      print('No Image Path Received');
    }
        ...
        Center(
            child: ListView(
                children: [
                            ...
                            child: Center(
                                child: ListView(
                                    children: [
                                      ...
                                      Container(
                                        child: photoLeftFrontTyreFile != null ? Image.file(photoLeftFrontTyreFile!, width: 150, height: 150, fit: BoxFit.fitWidth,)
                                            :
                                        IconButton(
                            ...
                            child: Center(
                                child: ListView(
                                    children: [
                                      ...
                                      Container(
                                        child: photoRightFrontFile != null ? Image.file(photoRightFrontFile!, width: 150, height: 150, fit: BoxFit.fitWidth,)
                                            :
                                        IconButton(
                                          alignment: Alignment.center,
                                          icon: const Icon(Icons.camera_alt_rounded, size: 30, color: secondaryOverlayColor),
                            ...
                            child: Center(
                                child: ListView(
                                    children: [
                                      ...
                                      Container(
                                        child: photoLeftBackTyreFile != null ? Image.file(photoLeftBackTyreFile!, width: 150, height: 150, fit: BoxFit.fitWidth,)
                                            :
                                        IconButton(
                                          alignment: Alignment.center,
                                          icon: const Icon(Icons.camera_alt_rounded, size: 30, color: secondaryOverlayColor),
                  ...
                  if(photoLeftFrontTyreFile != null)
                    if(photoRightFrontFile != null)
                      if(photoLeftBackTyreFile != null)
                        if(photoRightBackTyreFile != null)
                  SizedBox(
                    height: buttonHeight,
                    width: buttonWidthTwo,
                    child: OutlinedButton(
                      onPressed: () {
                        Navigator.pushNamed(context, '/ClientVehicleInteriorPage');
                      },
                      style: ButtonStyle(
